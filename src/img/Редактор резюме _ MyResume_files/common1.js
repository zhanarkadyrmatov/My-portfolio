...
$(document).ready(function(){ 
	...
	$('.modalLink[data-modal="#paymentWindow"]').off('click.step').on('click.step',function(e){
		e.preventDefault();
...
function spinner(sel,state) { 
	if(state === true) {
		$(sel).prop('disabled', state).parent().find('div.spin').addClass("icon-spinner-div")
		$('#paymentWindowColon .buyButton').css("opacity",0.5).attr('disabled','disabled')
	} else {
		'#paymentWindowColon .buyButton'
		$(sel).prop('disabled', state).parent().find('div.spin').removeClass("icon-spinner-div")
		$('#paymentWindowColon .buyButton').css("opacity",1).removeAttr('disabled')
	}
}

function initBuyButton() {
	$('#paymentWindow .buyButton').off('click').click(function(e){ e.preventDefault();
	...
	$('#paymentWindowColon .buyButton').off('click').click(function(e){ e.preventDefault();
		spinner(this,true);
		renderForm($(this).attr('data-id'), 0);
	});
